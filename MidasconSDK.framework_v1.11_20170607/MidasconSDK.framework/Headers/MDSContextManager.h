...
/**
 BLE Module을 시스템에서 재시작 할 수 있도록 App의 applicationLaunchingWithOptions 메소드에서 호출 하도록 한다.
 */
+ (void)applicationLaunchingWithOptions:(NSDictionary *)launchOptions;
+ (void)applicationLaunchingWithOptions:(NSDictionary *)launchOptions delegate:(id<MDSContextManagerDelegate>)delegate;

/**
 BLE Module을 시스템에서 재시작 할 수 있으려면 CoreBluetooth를 시작할 때 identifier 값이 있어야 한다. 만일 app에서 설정하지 않으면 내부에서 자동으로 초기화 한다.
 */
+ (void)setRestoreIdentifier:(NSString *)restoreIdentifier;
...
/**
 최소 beacon 수신 정보 전달 시간 간격을 설정한다. 기본값은 0.01초. 설정된 시간 사이 내에 다시 들어온 비콘 신호는 delegate로 전달되지 않는다. 이 값은 MIDASCON beacon type일 경우만 적용된다.
 */
+ (void)setMinimumAdvertisedTimeInterval:(NSTimeInterval)minimumInterval;
